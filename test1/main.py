...
# Persistent history of real scans, in the same per-card shape as
# CONNECTIONS_DATABASE / SOCIAL_DATABASE above — written to on every
# revealed scan, read back by the frontend so /network shows real people
# instead of only the hardcoded demo list.
CONNECTIONS_LOG_PATH = os.path.join(os.path.dirname(__file__), "connections_log.json")


def _load_connections_log():
    if not os.path.exists(CONNECTIONS_LOG_PATH):
        return []
    with open(CONNECTIONS_LOG_PATH, "r", encoding="utf-8") as fh:
        return json.load(fh)


def _save_connections_log(entries):
    with open(CONNECTIONS_LOG_PATH, "w", encoding="utf-8") as fh:
        json.dump(entries, fh, indent=2)


def _append_connection_entry(result):
    """Shapes a revealed evaluate_photo() result into a CONNECTIONS_DATABASE
    (formal) or SOCIAL_DATABASE (social) entry — same fields the frontend's
    card pages already expect — and appends it to connections_log.json.
    Only ever called for status == 'revealed', same consent lock as
    scan_logs: a match alone never gets written here either.
    """
    user_id = result["user_id"]
    now = datetime.now().strftime("%I:%M %p").lstrip("0")

    if result["mode"] == "formal_profile":
        ...
        entry = {
            ...
            "info_boxes": card.get("info_boxes", {}),
        }
    else:
        ...
        entry = {
            "id": card["id"],
            "type": "social",
            "type_label": "SOCIAL PROFILE",
            "time": now,
            "name": card.get("name"),
            "location": card.get("location"),
            "avatar": card.get("avatar"),
            "strength": card.get("strength"),
            "dating_me": card.get("dating_me"),
            "life_goal": card.get("life_goal"),
            "heart": card.get("heart"),
        }

    entries = _load_connections_log()
    entries.insert(0, entry)  # most recent first, same order /network shows
    _save_connections_log(entries)
...
def get_connections_log():
    """The real scan history — every revealed scan, most recent first, in
    the same shape /network's cards already render. Empty until someone's
    actually been scanned."""
    return _load_connections_log()
...
async def save_photo(request: Request):
    ...
    if result["status"] == "revealed":
        ...
        _append_connection_entry(result)
